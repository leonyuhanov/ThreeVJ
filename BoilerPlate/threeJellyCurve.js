import * as THREE from 'three';
import envelopGenerator from './envelopGenerator.js';					//Envelop Generator
import CCGenerator from './CCGenerator.js';								//Colour System
import timerObject from './timerObject.js';								//Timers
import pixelMaper from './pixelMaper.js';								//Pixel Maper
import animationObject from './animationObject.js';						//Generic Object Tracking class
import ElipticalEnvelopGenerator from './ElipticalEnvelopGenerator.js';
import curvePoint from './threeCurvePoint.js';
import { LineMaterial } from 'three/addons/lines/LineMaterial.js';
import { LineSegments2 } from 'three/addons/lines/LineSegments2.js';
import { LineSegmentsGeometry } from 'three/addons/lines/LineSegmentsGeometry.js';
import { Line2 } from 'three/addons/lines/Line2.js';
import { LineGeometry } from 'three/addons/lines/LineGeometry.js';

class threeJellyCurve
{	
	constructor()
	{
		//Main object stores
		this.objectTape = new Array();
		this.globalGroupArray = new Array();
		this.globalObjectGroup = new THREE.Object3D();
		this.groupName = "JC_";
		this.objectIDIndex = 0;
		this.genObject = new animationObject();
		
		//Global Three Objects form main System
		this.scene;
		
		//Main properties
		this.origin = [0,0,0];
		this.pointList = new Array();		//use curvePoint(radius, angle, minRadius, maxRadius)
		this.pollyFitness = 100;
		this.lineThickness = new Array();
		this.lines = 8;
		this.lfoSeed = new Array();
		this.opacity = 1;
		this.bloomEvery = 5;
		this.bloomEnable = new Array();
		this.pathData = [100,100,1];
		this.rotations = [0,0,0];
		this.multiObject = 0;
		this.setUpStatus = 0;
		
		//Utility Objects
		this.pixelMap = new pixelMaper(2,2);
		this.screenRange = [100,100,100];
		this.timers = new timerObject();
		this.envelops = new envelopGenerator();
		this.lfo = new ElipticalEnvelopGenerator();
		this.directionalVectors = [1,1,1];
		this.rotateTo = [0,0,0];
		this.modify = 0;
		
		//Colour System
		this.colourIndex = 0;
		this.subColourIndex = 0;
		this.creationColourIncrement = 10;
		this.maxValue = 255;
		this.maxColourDitherSteps = 128;
		this.colourList_1 = [this.maxValue,0,0,this.maxValue,this.maxValue,0, 0,this.maxValue,0, 0,this.maxValue,this.maxValue, 0,0,this.maxValue, this.maxValue,0,this.maxValue, this.maxValue,this.maxValue,this.maxValue];
		this.colourObject = new CCGenerator(this.maxColourDitherSteps, this.colourList_1.length/3, this.colourList_1);
	}
	init = function(scene, colourIndex)
	{
		var lineIndex=0, pointListArray, lastPointHeight, rpLength=0;
		
		this.scene = scene;
		this.colourIndex = colourIndex;
		for(lineIndex=0; lineIndex<this.lines; lineIndex++)
		{
			pointListArray = new Array();
			//vertex generation
			pointListArray.push(new curvePoint(0,0,0,0));
			//pointListArray.push(new curvePoint(50,100,100,110));
			//pointListArray.push(new curvePoint(80,100,100,110));
			//pointListArray.push(new curvePoint(100,120,120,130));
			//pointListArray.push(new curvePoint(200,100,100,120));
			pointListArray.push(new curvePoint(10,100,100,110));
			pointListArray.push(new curvePoint(40,100,100,110));
			pointListArray.push(new curvePoint(90,120,120,130));
			pointListArray.push(new curvePoint(120,100,100,120));
			//lastPointHeight = ((this.lfo.read("LineLength", 1, 0)/100)*150)+250;
			lastPointHeight = ((this.lfo.read("LineLength", 1, 0)/100)*150)+120;
			pointListArray.push(new curvePoint(lastPointHeight,120,120,160));
			
			this.pointList.push(pointListArray);
			//Line thickness
			this.lineThickness.push( ((this.lfo.read("LineThickness", 1, 0)/100)*5)+0.5 );
			//bloom
			if(lineIndex%this.bloomEvery==this.bloomEvery-1)
			{
				this.bloomEnable[lineIndex] = 1;
			}
			this.lfoSeed.push( this.lfo.read("LFOVariations", 1, 0) );
			//Radial pulse lfo
			rpLength = 200;
			this.lfo.addWithTimeCode("globRadPulse_"+lineIndex, [100], [rpLength], (lineIndex/this.lines)*rpLength, 0);
		}
	}
	animate = function(colourControls, controlData, rotationalIncrements=[0,0,0])
	{
		//controlData[0]  object scale
		//controlData[1]  width scale
		//controlData[2]  height scale
		//controlData[3]  depth scale
		//controlData[4]  vertex rotation scaler
		//controlData[5]  line width scale
		//controlData[6]  angular LFO
		//controlData[7]  radial LFO
		//controlData[8]  Global path speed
		
		if(this.setUpStatus==0){return;}

		var objectIndex=0, partIndex=0, vertIndex=0;
		var vertecies, tempCurve, vertArray;
		var localAngle=0, localRad=0;
		var pathPointPos = [0,0];
		
		for(objectIndex=0; objectIndex<this.objectTape.length; objectIndex++)
		{
			vertecies = new Array();
			for(vertIndex=0; vertIndex<this.pointList[objectIndex].length; vertIndex++)
			{
					if(vertIndex!=0)
					{
						//Angular LFO
						localAngle = this.pointList[objectIndex][vertIndex].angleRange * (this.lfo.read("O_"+objectIndex+"_V_"+vertIndex, controlData[6]+0.0001, 0)/100);
						localAngle = this.pointList[objectIndex][vertIndex].minAngle + localAngle;
						//Radial/Radius LFO
						localRad = (this.lfo.read("globRadPulse_"+objectIndex, controlData[7]+0.0001, 0)/100)*50;
						this.pointList[objectIndex][vertIndex].setAngle(localAngle);
						this.pointList[objectIndex][vertIndex].setRadius( this.pointList[objectIndex][vertIndex].initRadius+localRad );
					}
					vertecies.push( new THREE.Vector3(this.pointList[objectIndex][vertIndex].position[0], this.pointList[objectIndex][vertIndex].position[1], this.pointList[objectIndex][vertIndex].position[2]) );
			}
			tempCurve = new THREE.CatmullRomCurve3( vertecies );
			vertecies = tempCurve.getPoints( this.pollyFitness );
			vertArray = new Array();
			for(vertIndex=0; vertIndex<vertecies.length; vertIndex++)
			{
				vertArray.push(vertecies[vertIndex].x, vertecies[vertIndex].y, vertecies[vertIndex].z)
			}
			//Geometries
			this.objectTape[objectIndex].geometry[partIndex].setPositions( vertArray );
			//line width
			this.objectTape[objectIndex].materials[partIndex].linewidth = this.lineThickness[objectIndex]*controlData[5];
			
			//Line colour
			this.colourObject.getColour( this.subColourIndex%this.colourObject._bandWidth );
			this.objectTape[objectIndex].materials[partIndex].color.r = this.colourObject._currentColour[0]/255;
			this.objectTape[objectIndex].materials[partIndex].color.g = this.colourObject._currentColour[1]/255;
			this.objectTape[objectIndex].materials[partIndex].color.b = this.colourObject._currentColour[2]/255;
			
			this.subColourIndex+=colourControls[1];	
		}

		//global Scale
		this.globalObjectGroup.scale.set(controlData[0]*controlData[1], controlData[0]*controlData[2], controlData[0]*controlData[3]);
		//GLobal path fly thourgh
		this.pathData[2] += controlData[8];
		pathPointPos = this.pixelMap.getElipticalPointsRaw(0, 0, this.pathData[0], this.pathData[1], this.pathData[2]);
		/*
		this.globalObjectGroup.position.set(pathPointPos[0], pathPointPos[1], 0);
		this.globalObjectGroup.setRotationFromEuler( new THREE.Euler( 0, 0, 0, 'XYZ' ));
		this.globalObjectGroup.rotateY( this.angleToRadian( 180 ) );
		this.globalObjectGroup.rotateZ( this.angleToRadian( 90-this.pathData[2] ) );
		*/
		this.globalGroupArray[0].position.set(pathPointPos[0], pathPointPos[1], 0);
		this.globalGroupArray[0].setRotationFromEuler( new THREE.Euler( 0, 0, 0, 'XYZ' ));
		//this.globalGroupArray[0].rotateY( this.angleToRadian( 180 ) );
		this.globalGroupArray[0].rotateZ( this.angleToRadian( this.pathData[2]-275 ) );
		//global rotation		
		this.globalObjectGroup.rotateX( this.angleToRadian(rotationalIncrements[0]*this.rotations[0]) );
		this.globalObjectGroup.rotateY( this.angleToRadian(rotationalIncrements[1]*this.rotations[1]) );
		this.globalObjectGroup.rotateZ( this.angleToRadian(rotationalIncrements[2]*this.rotations[2]) );


		this.colourIndex += colourControls[0];
		this.subColourIndex = this.colourIndex;
	}
	insertObject = function()
	{
		var objectIndex=0, partIndex=0, vertIndex=0;
		var vertecies, tempCurve, vertArray;
		var XYpointPos = [0,0], XZPointPos = [0,0];
		var localGroup = new THREE.Object3D();
		
		this.subColourIndex = this.colourIndex;
		for(objectIndex=0; objectIndex<this.lines; objectIndex++)
		{
			//init the LFOs for each point
			for(vertIndex=0; vertIndex<this.pointList[objectIndex].length; vertIndex++)
			{
				this.lfo.addWithTimeCode("O_"+objectIndex+"_V_"+vertIndex, [100], [100], 0, this.lfoSeed[objectIndex]);
			}
			partIndex=0;
			this.objectTape.push( new animationObject() );
			this.objectTape[objectIndex].objectID = this.groupName+this.objectIDIndex;
			//vertex generator for curve
			vertecies = new Array();
			for(vertIndex=0; vertIndex<this.pointList[objectIndex].length; vertIndex++)
			{
				vertecies.push( new THREE.Vector3(this.pointList[objectIndex][vertIndex].position[0], this.pointList[objectIndex][vertIndex].position[1], this.pointList[objectIndex][vertIndex].position[2]) );
			}
			tempCurve = new THREE.CatmullRomCurve3( vertecies );
			vertecies = tempCurve.getPoints( this.pollyFitness );
			vertArray = new Array();
			for(vertIndex=0; vertIndex<vertecies.length; vertIndex++)
			{
				vertArray.push(vertecies[vertIndex].x, vertecies[vertIndex].y, vertecies[vertIndex].z)
			}
			//Geometries
			this.objectTape[objectIndex].geometry.push(  new LineGeometry() );
			this.objectTape[objectIndex].geometry[partIndex].setPositions( vertArray );
			//Material
			this.objectTape[objectIndex].materials.push( new LineMaterial( {color: 0xffffff, linewidth: this.lineThickness[objectIndex], worldUnits: true, vertexColors: false, alphaToCoverage: false} ) );
			this.objectTape[objectIndex].materials[partIndex].transparent = true;
			this.objectTape[objectIndex].materials[partIndex].opacity = this.opacity;
			//colour
			this.colourObject.getColour( this.subColourIndex%this.colourObject._bandWidth );
			this.objectTape[objectIndex].materials[partIndex].color.r = this.colourObject._currentColour[0]/255;
			this.objectTape[objectIndex].materials[partIndex].color.g = this.colourObject._currentColour[1]/255;
			this.objectTape[objectIndex].materials[partIndex].color.b = this.colourObject._currentColour[2]/255;
			//Object
			this.objectTape[objectIndex].objects.push( new Line2(this.objectTape[objectIndex].geometry[partIndex], this.objectTape[objectIndex].materials[partIndex]) );
			//rotate based on objects index around a circle
			this.objectTape[objectIndex].objects[partIndex].rotateY( this.angleToRadian( (360/this.lines)*objectIndex ) );
			if(this.bloomEnable[objectIndex]==1)
			{
				this.objectTape[objectIndex].objects[partIndex].layers.enable( 1 );
			}
			//add to local group
			localGroup.add( this.objectTape[objectIndex].objects[partIndex] );
			
			this.objectIDIndex++;
			this.subColourIndex += this.creationColourIncrement;
		}
		//reset colour index
		this.subColourIndex = this.colourIndex;
		
		//rotations
		localGroup.rotateX( this.angleToRadian( this.rotateTo[0] ) );
		localGroup.rotateY( this.angleToRadian( this.rotateTo[1] ) );
		localGroup.rotateZ( this.angleToRadian( this.rotateTo[2] ) );
		//setup global random axis rotations
		this.generatedirectionalVectors();
		this.rotations[0] = this.directionalVectors[0];
		this.rotations[1] = this.directionalVectors[1];
		this.rotations[2] = this.directionalVectors[2];
		//finalize objects
		this.globalGroupArray.push( localGroup );
		//console.log(this.globalGroupArray);
		//this.globalObjectGroup.add( localGroup );
		this.globalObjectGroup.attach( localGroup );
		//console.log(this.globalObjectGroup);
		
		//Finalize position
		this.globalObjectGroup.position.x =  this.origin[0];
		this.globalObjectGroup.position.y =  this.origin[1];
		this.globalObjectGroup.position.z =  this.origin[2];
		
		//add to global scene
		if(this.multiObject==0)
		{
			this.scene.add( this.globalObjectGroup );
		}
		this.setUpStatus = 1;
	}
	generatedirectionalVectors = function()
	{
		if( Math.round(Math.random()) == 1 ){this.directionalVectors[0]=1;}else{this.directionalVectors[0]=-1;}
		if( Math.round(Math.random()) == 1 ){this.directionalVectors[1]=1;}else{this.directionalVectors[1]=-1;}
		if( Math.round(Math.random()) == 1 ){this.directionalVectors[2]=1;}else{this.directionalVectors[2]=-1;}
	}
	seed = function(originPoint)
	{
		if(originPoint==undefined)
		{
			this.origin[0] = (-this.screenRange[0])+Math.round(Math.random()*(this.screenRange[0]*2));
			this.origin[1] = (this.screenRange[1])-Math.round(Math.random()*(this.screenRange[1]*2));
			this.origin[2] = (-this.screenRange[2])+Math.round(Math.random()*(this.screenRange[2]*2));
		}
		else
		{
			this.origin[0] = originPoint[0];
			this.origin[1] = originPoint[1];
			this.origin[2] = originPoint[2];
		}
		this.insertObject();
	}
	angleToRadian = function(angle)
	{
		return (angle%360)*(Math.PI/180);
	}
	angleToFloatAngle = function(angle)
	{
		return (angle%360)/360;
	}
	floatAngleToAngle = function (floatAngle)
	{
		return floatAngle*360;
	}
}
export default threeJellyCurve;