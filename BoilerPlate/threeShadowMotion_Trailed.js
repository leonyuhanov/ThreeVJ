...
{	
	constructor()
	{
		...
		this.trailPointCount = 4;
		this.copyVerts = new Array();
		this.shadowCount = 2;
		this.motionSpeed = 1;
		this.lfoSeed = 0;
	...
	animate = function(colourControls, controlData, rotationalIncrements=[0,0,0])
	{
		...
		var objectIndex=0, partIndex=0, pointPos, currentPostion=0;
		var objectPos;
		...
		{
			currentPostion = Math.abs( (this.objectTape[objectIndex].pointAngleOffset-(controlData[4]*partIndex))/this.pollyFitness );
			objectPos = this.objectTape[objectIndex].shape[0].getPointAt(currentPostion%1);
			this.objectTape[objectIndex].objects[partIndex].position.set(objectPos.x, objectPos.y, objectPos.z);
			//rotation
	...
	insertObject = function()
	{
		...
		var vertecies = new Array(), pointPos;
		var localGroup = new THREE.Object3D();
		var localScale = 0;
		
		//create trail path for animation
		if(this.copyVerts.length==0)
		{
			for(vertIndex=0; vertIndex<this.trailPointCount; vertIndex++)
			{
				pointPos = this.pixelMap.getElipticalPointsRaw(0, 0, this.dimensions[0], this.dimensions[1], Math.random()*360);
				if(vertIndex%2==0)
				{
					//XY Point Selection
				...
				{
					//ZY point Selection
					vertecies.push( new THREE.Vector3(0, pointPos[1], pointPos[1]) );
				}
			}
