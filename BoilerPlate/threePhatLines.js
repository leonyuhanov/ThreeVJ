...
{	
	...
	init = function(scene, colourIndex)
	{
		...
		this.lfo.addWithTimeCode("lineWidthLFO", [100], [100], 0, 0);
		this.lfo.addWithTimeCode("pollyRotationLFO", [100], [100], 0, 0);
	}
	animate = function(colourControls, controlData, rotationalIncrements=[0,0,0])
	{
		...
		//controlData[5]  line width envelop increment
		//controlData[6]  line width envelop increment motion
		//controlData[7]  PR envelop increment
		//controlData[8]  PR envelop increment motion
		
		if(this.setUpStatus==0){return;}
		var objectIndex=0, partIndex=0;
		var currentTimeCode = this.lfo.getTimeCode("lineWidthLFO");
		var startTimeCode = currentTimeCode;
		var prTimeCode = this.lfo.getTimeCode("pollyRotationLFO");
		var prStartTimeCode = prTimeCode;
		var localLineWidth=0, localPRangle = 0;
		
		for(objectIndex=0; objectIndex<this.objectTape.length; objectIndex++)
		{
			localLineWidth = this.lfo.read("lineWidthLFO", 0 ,currentTimeCode)/100;
			this.objectTape[objectIndex].materials[partIndex].linewidth = (controlData[4]*localLineWidth)+0.5;
			currentTimeCode += controlData[5];
			//colour
			...
			//rotation
			localPRangle = (this.lfo.read("pollyRotationLFO", 0, prTimeCode)/100)/4;
			prTimeCode += controlData[7];
			this.objectTape[objectIndex].objects[partIndex].rotateZ( this.angleToRadian(localPRangle) );			
		}
		this.lfo.setTimeCode("lineWidthLFO", startTimeCode+controlData[6]);
		this.lfo.setTimeCode("pollyRotationLFO", prStartTimeCode+controlData[8]);
	...
	insertObject = function()
	{
		...
		var zStart = -this.dimensions[2];
		
		this.zSpace = (this.dimensions[2]*2)/this.pollyCount;
		...
		for(objectIndex=0; objectIndex<this.pollyCount; objectIndex++)
		{
			...
			//vertex generator
			vertecies = new Array();
			for(vertIndex=0; vertIndex<this.linePoints; vertIndex++)
			{
				pointPos = this.pixelMap.getElipticalPointsRaw(0, 0, this.dimensions[0], this.dimensions[1], (360/this.linePoints)*vertIndex);
				vertecies.push(pointPos[0], pointPos[1], zStart);
			}
			...
			//Material
			this.objectTape[objectIndex].materials.push( new LineMaterial( {color: 0xffffff, linewidth: 2, worldUnits: true, vertexColors: false, alphaToCoverage: true} ) );
