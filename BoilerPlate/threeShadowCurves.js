...
{	
	...
	init = function(scene, colourIndex)
	{
		...
		for(vertexCounter=0; vertexCounter<this.vertecies; vertexCounter++)
		{
			...
			this.vertexPoints.push(new curvePoint(this.radius+(this.directionalVectors[0]*(Math.random()*this.radiusWobble)),(360/this.vertecies)*vertexCounter,0,360));
		}
	...
	animate = function(colourControls, controlData, rotationalIncrements=[0,0,0])
	{
		...
		//controlData[8]  Shadow Depth Scale
		...
		vertecies = new Array();
		for(vertIndex=0; vertIndex<this.vertexPoints.length; vertIndex++)
		{
			currentRadialLFOIndex += controlData[6];
			currentRadius = (this.lfo.read("radialLFO", 0, currentRadialLFOIndex)/100)*this.radiusWobble;
			this.vertexPoints[vertIndex].setRadius(this.vertexPoints[vertIndex].initRadius+currentRadius);
			this.shadowPoints[vertIndex].setRadius((this.vertexPoints[vertIndex].initRadius+currentRadius)*this.shadowScale);
			if(vertIndex+1<this.vertexPoints.length)
			{
				vertecies.push( new THREE.Vector3(this.vertexPoints[vertIndex].position[0], this.vertexPoints[vertIndex].position[1], this.vertexPoints[vertIndex].position[2]) );
				shadowVertecies.push( new THREE.Vector3(this.shadowPoints[vertIndex].position[0], this.shadowPoints[vertIndex].position[1], this.shadowDepth*controlData[8]) );
			}
			else
			{
				vertecies.push( vertecies[0] );
				shadowVertecies.push( shadowVertecies[0] );
			}
		}
		...
		//shadow
		tempCurve = new THREE.CatmullRomCurve3( shadowVertecies );
		lineVertecies = tempCurve.getPoints( this.pollyFitness );
		vertArray = new Array();
		for(vertIndex=0; vertIndex<lineVertecies.length; vertIndex++)
		{
		...
		this.objectTape[objectIndex+1].geometry[partIndex].setPositions( vertArray )
		
		//line width
		this.objectTape[objectIndex].materials[partIndex].linewidth = this.lineThickness*controlData[5];
		this.objectTape[objectIndex+1].materials[partIndex].linewidth = this.lineThickness*2*controlData[5];
		//colour
		...
		//shadow
		this.objectTape[objectIndex+1].materials[partIndex].color.r = this.colourObject._currentColour[0]/255;
		this.objectTape[objectIndex+1].materials[partIndex].color.g = this.colourObject._currentColour[1]/255;
		this.objectTape[objectIndex+1].materials[partIndex].color.b = this.colourObject._currentColour[2]/255;
	...
	insertObject = function()
	{
		...
		for(vertIndex=0; vertIndex<this.vertexPoints.length; vertIndex++)
		{
			vertecies.push( new THREE.Vector3(this.vertexPoints[vertIndex].position[0], this.vertexPoints[vertIndex].position[1], this.vertexPoints[vertIndex].position[2]) );
			//set up the shadow
			this.shadowPoints.push( this.vertexPoints[vertIndex] );
			//increase radius
			this.shadowPoints[vertIndex].setRadius(this.shadowPoints[vertIndex].radius*this.shadowScale);
		...
		for(vertIndex=0; vertIndex<this.vertexPoints.length; vertIndex++)
		{
			vertecies.push( new THREE.Vector3(this.shadowPoints[vertIndex].position[0], this.shadowPoints[vertIndex].position[1], this.shadowPoints[vertIndex].position[2]) );
		}
		tempCurve = new THREE.CatmullRomCurve3( vertecies );
		lineVertecies = tempCurve.getPoints( this.pollyFitness );
		vertArray = new Array();
		for(vertIndex=0; vertIndex<lineVertecies.length; vertIndex++)
		{
			vertArray.push(lineVertecies[vertIndex].x, lineVertecies[vertIndex].y, this.shadowDepth)
		}
		//Geometries
		this.objectTape[objectIndex].geometry.push(  new LineGeometry() );
		this.objectTape[objectIndex].geometry[partIndex].setPositions( vertArray );
		//Material
		this.objectTape[objectIndex].materials.push( new LineMaterial( {color: 0xffffff, linewidth: this.lineThickness*2, worldUnits: true, vertexColors: false, alphaToCoverage: false} ) );
		this.objectTape[objectIndex].materials[partIndex].transparent = true;
		this.objectTape[objectIndex].materials[partIndex].opacity = this.shadowOpacity;
		//colour
		this.colourObject.getColour( this.subColourIndex%this.colourObject._bandWidth );
		this.objectTape[objectIndex].materials[partIndex].color.r = this.colourObject._currentColour[0]/255;
		this.objectTape[objectIndex].materials[partIndex].color.g = this.colourObject._currentColour[1]/255;
...
export default threeShadowCurves;