...
class threeExtrudedCurvatureWalls
{	
	constructor()
	{
		//Main object stores
		...
		//Main properties
		this.origin = [0,0,0];
		this.dimensions = [200, 200, 5, 0.1];
		this.angularOffset = 0;
		this.angularSize = 180;
		this.startAngle = -90;
		this.rotationSpeed = (Math.random()*2)+0.2;
		this.lfoSeed = 0;
		this.bloomEnable = 0;
	...
	animate = function(colourControls, controlData, rotationalIncrements=[0,0,0])
	{
		...
		//controlData[4]  z rotation speed
		
		if(this.setUpStatus==0){return;}		
		
		this.globalObjectGroup.rotateZ( this.angleToRadian(this.rotationSpeed*controlData[4]) );
	...
	{
		var objectIndex=0, partIndex=0, shapeObject = new THREE.Shape();
		var pointPos, vertIndex, vertArray = new Array();
		var extrudeSettings = {steps: 2, depth: this.dimensions[2], bevelEnabled: false, bevelThickness: 1, bevelSize: 1, bevelOffset: 0, bevelSegments: 1};
		...
		for(vertIndex=0; vertIndex<this.angularSize; vertIndex++)
		{
			pointPos = this.pixelMap.getElipticalPointsRaw(0, 0, this.dimensions[0], this.dimensions[1], vertIndex+this.angularOffset);
			vertArray.push(new THREE.Vector2(pointPos[0], pointPos[1]));
		}
		for(vertIndex=this.angularSize; vertIndex>=0; vertIndex--)
		{
			pointPos = this.pixelMap.getElipticalPointsRaw(0, 0, this.dimensions[0]-(this.dimensions[0]*this.dimensions[3]), this.dimensions[1]-(this.dimensions[1]*this.dimensions[3]), vertIndex+this.angularOffset);
			vertArray.push(new THREE.Vector2(pointPos[0], pointPos[1]));
		}
		shapeObject.setFromPoints( vertArray );
		
		//Geometries
		this.objectTape[objectIndex].geometry.push(  new THREE.ExtrudeGeometry( shapeObject, extrudeSettings ) );
		//Material
...
export default threeExtrudedCurvatureWalls;