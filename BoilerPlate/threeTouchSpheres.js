...
{	
	...
	animate = function(colourControls, controlData, rotationalIncrements=[0,0,0])
	{
		...
		var objectIndex=0, partIndex=0, localScale=0;
		
		for(partIndex=0; partIndex<this.pointsPerObject; partIndex++)
		{
			localScale = (this.TOFArray[partIndex]/100);
			if(localScale>0)
			{
	...
	insertObject = function()
	{
		...
		for(partIndex=0; partIndex<this.pointsPerObject; partIndex++)
		{
			pointPos = this.pixelMap.getElipticalPointsRaw(0, 0, this.dimensions[0], this.dimensions[1], (360/this.pointsPerObject)*partIndex);
			this.objectTape[objectIndex].geometry.push( new THREE.SphereGeometry( this.sphereSize, 32, 32 ) );
			...
			this.objectTape[objectIndex].materials[partIndex].opacity = this.sphereOpacity;
			//colour
...
export default threeTouchSpheres;